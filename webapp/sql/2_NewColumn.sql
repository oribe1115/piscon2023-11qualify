...
(
    ...
    JOIN (SELECT `jia_isu_uuid`, MAX(timestamp) AS `latest_timestamp` FROM isu_condition GROUP BY jia_isu_uuid) AS t2 ON t.`jia_isu_uuid` = t2.`jia_isu_uuid` AND t.`timestamp` = t2.`latest_timestamp`
);
